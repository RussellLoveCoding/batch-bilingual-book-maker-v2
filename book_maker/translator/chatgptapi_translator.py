...
# PROMPT_HTML_TRANSLATOR = {
#     "user": """
# Imagine you are a professional translator. Please help me to translate the following text from {src_lang} to {target_lang},
# The text to translate is in html format, surrounded by triple backtick ```, you have to obey rules:
# 1. please keep the html format and translate the content in the tag.
# 2. only return translated text in html format, not including original text.
# ```html
# {text}
# ```
# """,
...
HTML_TRANSLATOR_USER_PROMPT_TEMPLATE = """Please help me to translate the following text in \
{format} format surrounded by triple backtick ``` from {src_lang} to {target_lang}. {ctx} \
...
class TranslatorPromptEditor():
    def __init__(self, format:str="html", src_lang:str="English", target_lang:str="Chinese", ctx=""):
        self.format = format
        self.src_lang = src_lang
        self.target_lang = target_lang
        self.user_content = HTML_TRANSLATOR_USER_PROMPT_TEMPLATE
        self.sys_content = f"You are a professional translator." 
        self.ctx = ctx
    ...
    def setLangPair(self, src_lang:str, target_lang:str):
        self.src_lang=src_lang
        self.target_lang=target_lang
        return self

    def fillTextGetUserPrompt(self, text:str):
        return self.user_content.format(
            format=self.format, src_lang=self.src_lang, target_lang=self.target_lang, text=text, ctx=self.ctx
        )

    def getSysMsg(self)->str:
        return self.sys_content

    def getUserMsg(self)->str :
...
class ChatGPTAPI(Base):
    ...
    def create_chat_completion(self, text, sys_content="", user_content=""):
        if user_content == "":
...
""",
    ...
    def get_best_result_list(
        ...
        """
        翻译多次，直到翻译结果的段落数量等于原始段落数量

        对失败进行重试，如何判断成功呢。

        如何改善翻译质量, 带个上下文，同时输出 json 格式就行了。
    ...
    def getHtmlTranslatorFunc(self) -> Callable[[str], str]:

        def translate_html_text(text) -> str:
            """
            prompt: 
            """
            # start_time = time.time()
            ...
            while attempt_count < max_attempts:
                try:
                    completion = self.create_chat_completion(text, PROMPT_HTML_TRANSLATOR["system"], PROMPT_HTML_TRANSLATOR["user"])
                    choice = completion["choices"][0]
    ...
    """)
        ...
        return translate_html_text


    def translate_html_text(self, textID:str, text:str, src_lang:str, target_lang:str, ctx: str) -> str:
        """
        prompt: 
        """
        # start_time = time.time()
        ...
        if len(text) > self.size:
            logger.error("text is too large, please split it")
            return  ""
        promptEditor = TranslatorPromptEditor(format="html",src_lang=src_lang, target_lang=target_lang, ctx=ctx)
        sys_content, user_content = promptEditor.getSysMsg(), promptEditor.fillTextGetUserPrompt(text)
        # logger.debug(promptEditor.user_content)
        # logger.debug(f"sys_content: {sys_content}, \nuser_content:  {user_content}")
        ...
        while attempt_count < max_attempts:
            try:
                completion = self.create_chat_completion(text,sys_content, user_content)
                ...
                if choice["finish_reason"] == "length":
                    print( logger.ERROR(
                        f"The total token is too long and cannot be completely translated. packedID: {textID} "))
                # break
            except Exception as e:
                if (
                    "choices" not in completion
