...
class EPUBBookLoaderHelper:
    ...
    def insert_trans(self, p, text, translation_style="", single_translate=False):
        """
        在原文段落 p 的后面加上 译文段落 text, 同时指定 翻译新段落的yangshi translation_style
        """
        if (
    ...
    def deal_old(self, wait_p_list:list, single_translate=False, context_flag=False):
        """
        对 wait_p_list 中每一个 段落 p 进行翻译，然后插入到 p 后面
        """

        if not wait_p_list:
            return

        # 只有 chatgpt 有上下文长度积累才会有这个 translate_list 这个方法
